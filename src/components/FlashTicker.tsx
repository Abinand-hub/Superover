...
export const FlashTicker: React.FC<FlashTickerProps> = ({
  ...
  // Duplicate the message for smooth seamless marquee loop
  ...
  return (
    <div className="w-full relative overflow-hidden rounded-xl bg-gradient-to-r from-[#0C142B] via-[#0E1B38] to-[#0A1024] border border-sky-500/30 shadow-lg shadow-black/40 p-1.5 sm:p-2 flex items-center gap-2.5 sm:gap-3 group select-none">
      ...
      <div className="relative z-10 shrink-0 flex items-center gap-1.5 px-3 py-1 sm:py-1.5 rounded-lg bg-gradient-to-r from-[#1D3A6B] via-[#254B8C] to-[#1E3B70] text-sky-100 border border-sky-400/40 font-black text-xs sm:text-sm shadow-md shadow-sky-950/60 tracking-tight">
      ...
      {/* Left/Right Edge Shadow Fade */}
      <div className="absolute left-[85px] sm:left-[110px] top-0 bottom-0 w-6 bg-gradient-to-r from-[#0C142B] to-transparent z-10 pointer-events-none"></div>
      ...
      <div className="overflow-hidden flex-1 relative py-0.5">
        <div className="animate-marquee whitespace-nowrap flex items-center cursor-default">
          <span className="text-[#FFE270] font-bold text-xs sm:text-sm tracking-wide drop-shadow-[0_0_12px_rgba(255,226,112,0.45)] mr-8">
            {repeatedText}
          </span>
          <span className="text-[#FFE270] font-bold text-xs sm:text-sm tracking-wide drop-shadow-[0_0_12px_rgba(255,226,112,0.45)] mr-8" aria-hidden="true">
            {repeatedText}
          </span>
        </div>
      </div>
